export class Task {
  ...
  function findOptimalSolution(tasksList: Task[]): Task[] {
  ...
  function printTaskList(tasksList: Task[]): void {
  ...
  function printLoadChart(tasksList: Task[]): void {
      ...
      for (let time = 0; time <= maxWidth; time++) {
          const fitTasks = tasks.filter(task => time >= task.earlyStart && time < task.earlyFinish);
          const load = fitTasks.reduce((sum, task) => sum + task.resources, 0);

          console.log(`Time: ${time} Load: ${load} Tasks: ${fitTasks.map(task => task.taskId).join(', ')}`);
      ...
      const maxLabelLength = Math.max(...timeToLoad.map(([time]) => time.toString().length));
      const labelFormat = (day: number) => day.toString().padStart(maxLabelLength, ' ');
  ...
  function getCriticalTaskChain(tasks: Task[]): Task[] {
  ...
  function sortTaskChain(tasks: Task[]): Task[] {
      ...
      while (visitedTasks.length !== tasks.length) {
          for (const task of tasks) {
              ...
              if (!visitedTasks.map(t => t.taskId).includesAll(nextTasks.map(t => t.taskId))) continue;
  ...
  function findProjectDuration(tasks: Task[]): number {
      ...
      while (visitedTasksId.length !== tasks.length) {
          for (let i = 0; i < tasks.length; i++) {
              ...
              if (nextTasks.length === 0) {
                  tasks[i] = new Task(task.taskId, task.duration, task.resources, task.previous, task.earlyStart, task.earlyFinish, task.isVirtual);
                  continue;
              }
              ...
              tasks[i] = new Task(task.taskId, task.duration, task.resources, task.previous, task.earlyStart, lateFinish, task.isVirtual);
          }
      }
  }
      ...
      while (visitedTasksId.length !== tasks.length) {
          for (let i = 0; i < tasks.length; i++) {
              ...
              if (task.previous.length === 0) {
                  tasks[i] = new Task(task.taskId, task.duration, task.resources, task.previous, 0, task.lateFinish, task.isVirtual);
                  continue;
              }

              // for handle tasks before
              const earlyStart = Math.max(...tasks.filter(t => task.previous.includes(t.taskId)).map(t => t.earlyFinish));

              tasks[i] = new Task(task.taskId, task.duration, task.resources, task.previous, earlyStart, task.lateFinish, task.isVirtual);
          }
      }
  }
  ...
  function makeStartTasksList(tasksList: Task[]): Task[] {
      ...
      const firstTasks = tasks.filter(task => task.previous.length === 0).map(task => task.taskId);
      ...
      const endTasks = tasks.filter(task => tasks.every(t => !t.previous.includes(task.taskId))).map(task => task.taskId);
